...
export async function execute(interaction) {
  try {
    ...
    if (!queue.paused) {
      const errEmbed = new EmbedBuilder().setDescription(`The queue is already playing`).setColor(env.ERROR_COLOR);
      return interaction.reply({ embeds: [errEmbed] });
    }

    queue.resume();
    const pauseEmbed = new EmbedBuilder().setDescription(`Resumed the queue`).setColor(env.MUSIC_COLOR);
