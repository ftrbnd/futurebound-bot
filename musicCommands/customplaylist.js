      ...
      if (interaction.options.getSubcommand() === 'create') {
        ...
        if (interaction.isAutocomplete()) {
          ...
          await Playlist.findOne({ name: playlist }, (err, data) => {
            ...
            if (!data) {
              const dataEmbed = new EmbedBuilder().setDescription(`**${playlist}** custom playlist does not exist`).setColor(process.env.ERROR_COLOR);
              ...
              if (voiceChannel) {
                ...
                const confirmEmbed = new EmbedBuilder().setDescription(`Now playing **${playlist}** in ${voiceChannel}`).setColor(process.env.MUSIC_COLOR);
                interaction.reply({ embeds: [confirmEmbed] });
              } else {
