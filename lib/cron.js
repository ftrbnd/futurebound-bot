...
async function snapshotNext(client) {
  try {
    ...
    if (status.prevDay === status.nextDay) {
      ...
      const retryButton = new ButtonBuilder().setCustomId('retry_daily_heardle').setLabel('Retry').setStyle(ButtonStyle.Primary);
...
// TODO: fix cron jobs not running at specified time
module.exports = {
  snapshotPrev,
  snapshotNext,
